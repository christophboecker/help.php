...
const DOC_MODE = 'docs';
const DEF_LANG = '';
const DEF_NAVI = '_navi.md';
const DEF_CONT = '_intro.md';

// read defaults and configuration
$defaults = array_merge (
    [YML_MODE=>DEF_MODE,YML_LANG=>DEF_LANG,YML_NAVI=>DEF_NAVI,YML_CONT=>DEF_CONT],
...
if ( ($image = rex_request( URL_IMAGE, 'string')) != '' )
{
    ...
    if( is_file($imagefile="$langPath$image") || is_file ( $imagefile="$docPath$image") ) {
        ...
        foreach( $fallback as $v ) {
            if( !is_file($imagefile="$docPath$v/$image" ) ) continue;
            rex_response::sendFile( $imagefile, mime_content_type( $imagefile ) );
            exit;
        }
    }
...
$docMode = $defaults[YML_MODE] === DOC_MODE && is_dir( $docPath );
...
if( $docMode )
{
    ...
    // get filename for navigation and content;
...
// Readme-Mode ------------------------------------------------------------------------------
    ...
    foreach( $matchNav as $k=>$v ) {
        $navigation .= str_repeat( " ", $v['level']-1 ) . "- [{$v['titel']}]($urlDoc$k#$marker$k)" . PHP_EOL;
    }

    // identify and clip out active Section
    if( ($section = rex_request( URL_FILE, 'string', 0 )) )  {
    ...
    // correct internal references to outside-chapters
...
echo '<section class="rex-docs rex-yform-docs"><div class="row">';
...
if( $navigation )        {
    ...
    echo '<div class="col-md-4 rex-docs yform-docs-navi">'.$fragment->parse('core/page/section.php').'</div>';
    echo '<div class="col-md-8 docs-content yform-docs-content">'.$content.'</div>';
} else {
    echo '<div class="col-md-12 docs-content yform-docs-content">'.$content.'</div>';
}
echo '</div></section>';
?>
<style>
